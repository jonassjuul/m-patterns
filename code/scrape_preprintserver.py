...
preprintserver = ['biorxiv','medrxiv'][1]
...
while (run < max_runs and number_of_papers == 100) :
    url = "https://api.biorxiv.org/details/"+preprintserver+"/%s/%s/"%(from_date,to_date)+str(article_num)
    ...
    with open(destination+preprintserver+'_FROM%s_TO%s.txt'%(from_date,to_date), 'a') as outfile:
        for element in data['collection'] :
